...
from ..utils.geometry import line_string_to_point_list, ensure_multi_point
...
def _generate_satin_guide_helper_lines_with_varying_pattern_distance(stroke, guide_line, outline, anchor_point, pattern_height, outline_rotation):
    # rotate pattern and get the pattern width
    first_rail = LineString(guide_line.rails[0])
    outline = rotate(outline, -outline_rotation, use_radians=True, origin=Point(anchor_point))
    minx, miny, maxx, maxy = outline.bounds
    anchor_point = InkstitchPoint(anchor_point.x, miny)
    pattern_width = maxy - miny

    distance = 0
    min_distance = stroke.get_float_param("min_line_dist_mm", 0)
    line_point_dict = defaultdict(list)
    while True:
        if distance > first_rail.length:
            break
        check_stop_flag()
        ...
        # move to point0, rotate and scale so the other point hits point1
        scaling = (point1 - point0).length() / pattern_height
        scaled_width = pattern_width * scaling

        # rotation
        circle = Point(point0).buffer(scaled_width)
        circle_intersections = ensure_multi_point(circle.boundary.intersection(first_rail))
        projections = []
        points = []
        for point in circle_intersections.geoms:
            projection = first_rail.project(point)
            if projection <= distance:
                continue
            projections.append(abs(distance + scaled_width - projection))
            points.append(point)
        if not points:
            break
        end_point = points[projections.index(min(projections))]
        point1 = InkstitchPoint(end_point.x, end_point.y)
        rotation = get_rotation(point0, point1) - 1/2 * pi

        # translation
        translation = point0 - anchor_point
        ...
        distance = max(distance + 0.1, first_rail.project(end_point) + min_distance)
...
def get_cut_points(guide_line, distance):
    cut_point = LineString(guide_line.rails[0]).interpolate(distance)
