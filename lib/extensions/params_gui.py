...
# params_gui.py — wx-dependent GUI classes for the Params extension.
# Separated from params.py to allow lazy loading (wx is heavy).
...
# Shared icon cache (loaded once, shared across all ParamsTab instances)
_icon_cache = {}


def _get_icon(name):
    """Load and cache an icon bitmap by name."""
    if name not in _icon_cache:
        _icon_cache[name] = wx.Image(
            os.path.join(get_resource_dir("icons"), name)).ConvertToBitmap()
...
class ParamsTab(ScrolledPanel):
    def __init__(self, *args, **kwargs):
        ...
        self.pencil_icon = _get_icon("pencil_20x20.png")

        randomize_icon = 'randomize_20x20.png'
        if wx.SystemSettings().GetAppearance().IsDark():
            randomize_icon = 'randomize_20x20_dark.png'
        self.randomize_icon = _get_icon(randomize_icon)
    ...
    def set_toggle_state(self, value):
        if self.toggle_checkbox and self.toggle:
            self.toggle_checkbox.SetValue(value)
            self.param_changed(self.toggle.name)
    ...
    def update_choice_widgets(self, choice_tuple=None):
        if choice_tuple is None:
            ...
            if last_selection != -1 and last_selection != current_selection:
...
class SettingsPanel(wx.Panel):
    def __init__(self, parent, nodes, tabs_factory=None, metadata=None, background_color='white', simulator=None):
        ...
        assert self.tabs_factory is not None, "tabs_factory is required"
        ...
        self.__do_layout()
        # Defer initial preview so the dialog appears immediately
