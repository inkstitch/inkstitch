...
class EmbroideryElement(object):
    def __init__(self, node):
        ...
        # convert legacy manual_stitch
        manual_stitch = self.get_param('manual_stitch', None)
        if manual_stitch == "True":
            self.set_param('stroke_method', 1)
            del self.node.attrib[INKSTITCH_ATTRIBS['manual_stitch']]
