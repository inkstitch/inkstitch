...
class AutoFill(EmbroideryElement):
    ...
    @property
    @param('fill_method', _('Fill method'), type='dropdown', default=0, options=[_("Auto Fill"), _("Tangential"), _("Manual")], sort_index = 2)
    ...
    @property
    @param('interlaced', _('Interlaced'), type='boolean', default=True,select_items=[('fill_method',1)], sort_index = 2)
    def interlaced(self):
        return self.get_boolean_param('interlaced', True)

    #@property
    #@param('fill_tangential', _('Tangential'), type='boolean', default=False, select_item=('fill_method',1), sort_index = 3)
    #def fill_tangetial(self):
    #    return self.get_boolean_param('fill_tangential', False)    
        ...
        try:
            ...
            if self.fill_method == 0:
                ...
                stitch_lists = legacy_fill(self.shape,
                                   ...
                                   self.staggers,
                                   self.skip_last)
                if self.fill_underlay and len(stitch_lists) > 0:
                    stitch_lists[0] = stitches+ stitch_lists[0]
