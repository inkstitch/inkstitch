...
def _resolve_log_dir(development_mode: bool, log_location: str,
                     SCRIPTDIR: Path) -> Path | None:
    ...
    match log_location.lower():
        ...
        case "document" | "doc":
            if doc_env := os.environ.get(
                    "DOCUMENT_PATH"):  # full name eg: .../x.svg
                doc_path = Path(doc_env).expanduser().resolve()
                logdir = doc_path.parent if doc_path.is_file() else doc_path
                print(f"DEBUG: Using document directory for logging: {logdir}",
                      file=sys.stderr)
                return logdir
            print(
                "DEBUG: DOCUMENT_PATH environment variable not set. Disabling logging.",
                file=sys.stderr)
            return None

        case "user" | "usr":
            from platformdirs import user_log_dir
            logdir = Path(user_log_dir("inkstitch")).expanduser().resolve()
            print(f"DEBUG: Using user directory for logging: {logdir}",
                  file=sys.stderr)
            return logdir
        ...
        case _:
            print(
                f"DEBUG: Unrecognized log_location '{log_location}'. Disabling logging.",
                file=sys.stderr)
            return None
...
def activate_logging(development_mode: bool, log_location: str,
    ...
    return LOGDIR  # return the directory where log are written
...
# "handlers": {
#    "file": {
#         "filename": "%(LOGDIR)s/xxx.log",     # <--- replace %(LOGDIR)s ->  script path
...
#   - we do: filename = "%(LOGDIR)s/inkstitch.log" -> filename = "path/inkstitch.log"
