...
Les fichiers qui composent une fonte du lettrage sont regroupés dans un dossier spécifique à la fonte, et qui est soit dans le dossier des fontes d'Ink/Stitch (fontes intégrées à Ink/Stitch) soit dans un [dossier personnel de fonte](fr/docs/font-tools/#custom-font-directory) pour les fontes personnelles de l'utilisateur.
...
Une police comme déjà vu comporte elle les deux fichiers →.svg et ←.svg, un texte multi ligne pourra être brodé en aller retour: dans le fichier →.svg, la broderie d'une lettre s'effectue de la gauche vers la droite tandis que dans le fichier ←.svg, elle s'éffectue de la droite vers la gauche. 
...
Les fontes du module de lettrage comportent aussi:
...
- un guide nommé "baseline" qui correspond a la ligne sur laquelle on écrit. Ink/Stitch a besoin de ce guide pour que le lettrage selon un chemin fonctionne correctement
...
Si l'on ne sait pas vraiment où sont les glyphes que l'on veut conserver, il peut être utile de procéder ainsi:
...
Ouvrir le fichier svg ainsi créé dans Inkscape.
