...
Le point triple décrit une répétition de points droits avant arrière. Il en résulte un point plus fourni.
...
2. Définissez le nombre de répétitions dans `Nombre de répétitions de points triples` dans [paramètres de point droit](/fr/docs/stitches/running-stitch).
   ...
   * Une valeur de 1 triple chaque point (avant, arrière, avant).
   * Une valeur de 2 quintuple chaque point, etc.
...
Forcer les points d'arrêts        |Force un point d'arrêt après l'objet indépendament de la valeur de "longueur minimum de saut" .
