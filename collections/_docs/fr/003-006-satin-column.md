...
Ink/Stitch offre plusieurs options pour créer des colonnes satin. La première méthode rend un trait en satin, sans conversion explicite, lesL méthodes 2 à 5 permettent une conversion vers une colonne satin standard qui pourra ensuite être modifiée manuellement.

![Méthodes](/assets/images/docs/satin_methods.svg)
1. [Trait rendu en satin](#1-trait-rendu-en-satin): pour créer des colonnes satin de largeur constante, sans conversion explicite en colonne satin
2. [Trait en satin](#2-trait-en-satin): pour créer des colonnes satin de largeur constante
3. [Trait en Effet de Chemin satin](#3-trait-en-effet-de-chemin-satin): colonne satin facilement modifiable avec un motif de contour optionnel
4. ["Ligne en zigzag" en Satin](#4-ligne-en-zigzag-en-satin): une manière simple de créer les colonnes satin particulièrement bien adaptée aux tablettes graphiques et aux écrans tactiles.
5. [Remplissage en satin](#5-remplissage-en-satin): pour créer des colonnes satin à partir de remplissage
...
* Défininir la largeur du contour à la taille souhaitée pour votre point satin (cette valeur doit être plus grande que le seuil d'auto conversion trait satin tel que défini dans les préférences ink/stitch, voir [preferences](/docs/fr/preferences/#minimum-satin-stroke-width) pour plus d'informations)
* Exécuter `Extensions > Ink/Stitch > Paramétres`.
* Ouvrir l'onglet « Colonne satin » et activer l'option « Colonnes de satin personnalisées ».

Avec cette méthode, la position des nœuds peut influencer le rendu du satin.
...
* Choisir un trait (un objet avec une couleur de contour, mais pas de couleur de remplissage)
* Définir l'épaisseur du contour à la largeur souhaitée pour votre colonne satin.
* Lancer `Extensions > Ink/Stitch > Outils : Satin > Trait en Satin`
...
* Utiliser tel quel ou modifier les traverses ou les rails en déplaçant leur noeuds

Plus d'informations sur [Convertir ligne en satin](/fr/docs/satin-tools/#convertir-ligne-en-satin)

### 3. Trait en Effet de Chemin Satin
...
Utiliser `Chemin > Objet en chemin` pour convertir en colonne satin standard (colonne satin manuelle).
...
### 6. Création manuelle de Colonne Satin
...
Si ce n'est pas le cas, ink/stitch essaye d'inverser automatiquement un des deux rails, mais il est préférable de controler soi même la direction des rails, pour cela, sélectionnez avec l' *Outil Éditer les noeuds* (`N`) un noeud d'un des deux rails et faites `Chemin > Inverser`. Cela n'inversera que le rail sélectionné. Vous pouvez aussi agir sur la direction des rails dans le paramétrage.
