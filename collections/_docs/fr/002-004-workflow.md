...
## ![Create Icon](/assets/images/docs/workflow-icon-create.png) Étape 1: Créer une image vectorielle
...
Modifier les objets et les chemins avec ces outils:
* ![outil de sélectionicon](/assets/images/docs/inkscape-tools-select.png) Outil de sélection (<key>S</key>) 
...
Lorsque vous basez un dessin sur une image ou un graphique existant, chargez-le dans Inkscape dans son propre calque. 

Certains graphiques sont compatibles avec [la fonction de traçage automatique](https://inkscape.org/fr/doc/tutorials/tracing/tutorial-tracing.html) d'Inkscape (**Chemin > Vectoriser une image matricielle** ou <key>Shift</key> <key>Alt</key> <key>B</key>), surtout si vous simplifiez d’abord l’image dans un éditeur graphique (par exemple avec [GIMP](https://www.gimp.org/)).

Après le traçage, nettoyez les formes vectorielles en utilisant **Chemin> Simplifier** (<key>Ctrl</key> <key>L</key>) et en supprimant les nœuds à la main lorsque cela est possible. Le but est d’utiliser le moins de courbes de Bézier possible pour représenter l’image. 
...
### Texte
Ink/Stitch vous offre la possibilité d'utiliser des polices prêtes à broder qui peuvent être insérées dans votre document via **Extensions > Ink/Stitch > Lettrage**
Vous pouvez insérer ces fontes dans votre document en sélectionnant **Extensions > Ink/Stitch > Lettrage**. 

Cet outil crée des objets textuels déjà optimisés pour la broderie.
...
## ![Vectorize](/assets/images/docs/workflow-icon-vectorize.png) Étape 2: Convertir en vecteurs pour la broderie et paramétrer
...
Ink/Stitch prend en charge plusieurs types de points. 

Ces trois principales catégories sont disponibles :
...
**Traits : points de contour**

Ces points suivent la direction d'un tracé (de préférence ouvert). Ils sont utilisés pour les contours, les lignes fines et les accents décoratifs, lorsqu'une ligne de point étroite est plus appropriée qu'une forme pleine.

**Points satin (ou  colonnes satin)**
...
Vous pouvez configurer le comportement des points via **Objet > Fond et Contour** ou en appuyant sur <key>Ctrl</key><key>Shift</key><key>F</key>. 

Consultez ce tableau et suivez les liens pour comprendre comment créer un type de point spécifique :
...
La boîte de dialogue **Extensions > Ink/Stitch > Paramètres** permet de contrôler la génération des points de broderie par Ink/Stitch pour les objets sélectionnés. Les paramètres varient selon le type de point et incluent des valeurs telles que la longueur, la densité, la sous-couche, la direction et la compensation d'étirement.
...
## ![Create Icon](/assets/images/docs/workflow-icon-order.png) Étape 3: Ordonner le plan de broderie  & Mettre des commandes

### Ordre de broderie

Lorsque vous concevez des motifs pour des machines à broder qui ne peuvent pas couper le fil au milieu de la broderie ou changer de couleur automatiquement, vous devez optimiser les chemins de points pour réduire ou masquer les sauts de points et effectuer le minimum de changements de fil. Essayez également d'éviter autant que possible de broder sur les sauts de points, car il serait alors très pénible de les couper à la main.
...
La couleur du fil est déterminée par la couleur de l'objet ; ainsi, un changement de couleur d'un objet à l'autre entraîne un changement de fil dans le fichier de broderie exporté.


Ink/Stitch vous offre plusieurs outils pour vous aider à optimiser l'ordre de broderie :
* La fenêtre **Calques et Objets** vous permet de voir l'ordre des objets dans la pile et de déplacer les objets dans cette pile
* **Extension  > Ink/Stitch > Édition > Réempiler les objets dans l'ordre de leur sélection** vous aide à réordonner manuellement des objets
* **Extension  > Ink/Stitch >Outils: Satin >Arrangement automatique de colonnes satins** appliqué après avoir sélectionné des colonnes satin, permet un réordonnnement automatique de ces colonnes satin, avec un eventuel ajout de chemins de dessous en point droit pour éviter les sauts de fils
* **Extension  > Ink/Stitch >Outils:Trait >Arrangement automatique de points droits**  ou **Extension  > Ink/Stitch >Outils:Trait >Redwork** appliqué après avoir sélectionné des points droits permet un réordonnnement automatique de ces point droits, avec un eventuel ajout de chemins de dessous en point droit pour éviter les sauts de fils. L'outil **Redwork** est à privilégier si la broderie nécessite très peu de sauts de fil , chaque chemin sera alors parcouru exactement deux fois.



**Astuce:** Inkscape vous permet de modifier l'ordre d'empilement dans le dialogue **Calques et Objets** avec les touches **Page précédente** et **Page suivante**. 
Pour une meilleure précision, il est conseillé d'associer les touches **Page précédente** et **Page suivante** à ces commandes. Cette configuration facilite la gestion de l'ordre des points lors de la préparation d'un motif de broderie. Consultez la section [Raccourcis clavier](/fr/docs/customize/#shortcut-keys) pour plus d'informations.

**Info:** Vous pouvez également modifier manuellement la structure XML SVG sous-jacente à l'aide du dialogue Editeur XML d'Inkscape (<key>CTRL</key> <key>SHIFT</key> <key>X</key>). Ses boutons "Monter" et "Descendre" agissent directement sur l'ordre des balises XML dans le fichier SVG et ne sont pas soumis aux mêmes limitations que les Page haut et Page bas d'origine. Notez que l'ordre des balises XML dans l'outil  d'édition XML  est l'_inverse_ de l'ordre des objets dans l'outil Objects.
{: .notice--info }

### Commandes
[Commandes](/fr/docs/commands/) aide également à optimiser votre chemin de points. 

Ces outils permettent de définir les points de départ et d'arrivée, de déplacer le cadre à broder et d'ajouter des instructions de coupe. 

L'utilisation des commandes offre un contrôle plus précis sur la broderie. Elles permettent de définir les points de départ et d'arrivée, de contrôler les coupes, les sauts, les arrêts et les changements de couleur, et de déplacer le cadre à broder. 

L'utilisation des commandes contribue à réduire sauts de fil visibles, à limiter les coupes manuelles et à améliorer la fluidité générale de la broderie.



## ![Create Icon](/assets/images/docs/workflow-icon-visualize.png)  Étape 4: Visualiser
...
Il y a aussi une visualiation lors de l'application du paramétrage, mais elle se limite à un unique type de points, celui que vous êtes en train de paramétrer. Elle ne permet généralement pas de visualiser l'ensemble de la broderie.




## ![Create Icon](/assets/images/docs/workflow-icon-export.png) Étape 5: Enregistrer le fichier broderie
...
**N'oubliez pas de sauvegarder également votre fichier au format SVG. Sinon, il sera difficile ou impossible de changer les détails plus tard.**

## ![Create Icon](/assets/images/docs/workflow-icon-testsew.png) Étape 6: Test de broderie
...
## ![Create Icon](/assets/images/docs/workflow-icon-optimize.png) Étape 7+: Optimisation
