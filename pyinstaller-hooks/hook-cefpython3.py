...
def get_cefpython3_binaries():
    binaries = []

    if is_linux:
        binaries.append((CEFPYTHON3_DIR + "/libcef.so", ""))

        # pyinstaller misses these because they're loaded by nss at runtime
        for file in glob.glob('/usr/lib/*/nss/*.so'):
...
def get_cefpython3_datas():
    ...
    for filename in os.listdir(CEFPYTHON3_DIR):
        ...
        if filename[-4:] in [".exe", ".dll", ".pak", ".dat", ".bin",
                             ".txt"]\
                or filename in ["License", "subprocess"]:
            logger.info("Include cefpython3 data: %s" % filename)

            if is_linux:
                ret.append((os.path.join(CEFPYTHON3_DIR, filename), "cefpython3"))
            else:
...
# Include binaries
binaries = get_cefpython3_binaries()
